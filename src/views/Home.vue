        ...
        <template v-for="(item, index) in contentList">
          <v-row :key="index">
            <template v-for="(data, i) in item.list">
              <hero-card v-if="item.type === 0" :key="i" :item="data" />
            </template>
          </v-row>
          <v-row v-if="item.type === 1" :key="index">
            <v-col v-for="(data, i) in item.list" :key="i" cols="12" md="6" class="pa-8">
              <hero-card :key="i" :item="data" />
            </v-col>
          </v-row>

          <v-row :key="index" class="mt-8">
            <section-header v-if="index === 1" title="FEATURED ARTICLES" />
          </v-row>
          <v-row :key="index">
            <template v-for="(data, i) in item.list">
              <card v-if="item.type === 2" :key="i" :item="data" />
            </template>
          </v-row>
        </template>
      </v-col>
...
export default {
  ...
  components: {
    ...
    Toast: () =>
      import(
        /* webpackChunkName: "home" */ "@/components/Common/Toast.vue"
      )
  },
  data() {
    return {
